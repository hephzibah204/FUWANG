...
use App\Models\AuctionAdminAuditLog;
use Closure;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Symfony\Component\HttpFoundation\Response;

class AuctionAdminAuditMiddleware
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        try {
            ...
            AuctionAdminAuditLog::create([
                'auction_admin_id' => $admin?->id,
                'action' => $routeName ?: ('auction_admin.' . strtolower($request->method()) . '.' . str_replace('/', '.', $request->path())),
                'meta' => $meta,
                'ip' => $request->ip(),
                'user_agent' => substr((string) $request->userAgent(), 0, 1000),
            ]);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::error('AuctionAdminAuditMiddleware: failed to write audit log', [
                'error' => $e->getMessage(),
                'route' => $routeName ?? null,
                'auction_admin_id' => ($admin ?? null)?->id,
                'method' => $request->method(),
                'path' => $request->path(),
            ]);
        }
