...
/**
 * PayVessel Payment Gateway Integration Service
 * 
 * Documentation: PayVessel API Documentation
 * 
 * Prerequisites:
 * - PayVessel merchant account with API credentials
 * - API key and secret key from PayVessel dashboard
...
class PayVesselService
{
    ...
    public function __construct(array $config)
    {
        ...
        // PayVessel base URL (sandbox or live)
        $this->baseUrl = $this->isLiveMode() 
            ? 'https://api.payvessel.com' 
            : 'https://sandbox-api.payvessel.com';
    }
    ...
    private function getAccessToken(): ?string
    {
        if (empty($this->apiKey) || empty($this->secretKey)) {
            Log::warning('PayVessel API credentials not configured');
            return null;
        }
        ...
        try {
            ...
            Log::error('PayVessel auth failed', [
            ...
            Log::error('PayVessel auth exception', ['error' => $e->getMessage()]);
            return null;
        }
    }

    /**
     * Initialize PayVessel checkout session
     * 
    ...
    public function initializeCheckout(
        ...
        if (!$token) {
            return [
                'ok' => false,
                'message' => 'Unable to authenticate with PayVessel'
            ];
        }
        ...
        $payload = [
            ...
            'redirect_url' => url('/payment/callback/payvessel'),
            'callback_url' => url('/webhooks/payvessel'),
        ...
        try {
            ...
            Log::error('PayVessel checkout initialization failed', [
                'status' => $response->status(),
                'body' => $response->json(),
                'payload' => $payload
            ]);
            ...
            Log::error('PayVessel checkout exception', [
                'error' => $e->getMessage(),
                'payload' => $payload
            ]);
    ...
    public function verifyTransaction(string $reference): array
    {
        ...
        try {
            ...
            Log::error('PayVessel transaction verification failed', [
                'status' => $response->status(),
                'body' => $response->json(),
                'reference' => $reference
            ]);
            ...
            Log::error('PayVessel verification exception', [
                'error' => $e->getMessage(),
                'reference' => $reference
            ]);
    ...
    public function handleWebhook(array $payload): array
    {
        ...
        $signature = request()->header('X-PayVessel-Signature');
    ...
    public function getSdkConfig(): array
    {
        if (empty($this->apiKey)) {
            return [
                'ok' => false,
                'message' => 'PayVessel API key not configured'
            ];
        }
