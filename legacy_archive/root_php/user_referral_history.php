...
// Count transactions where email matches and funding_type is referral_commission
$sql_count = "SELECT COUNT(*) AS total_transactions FROM funding_history WHERE email = ? AND funding_type = 'Referral commission'";
...
// Debug: Check the number of transactions counted
echo "";
...
<h1>
    <nobr>
<strong style="width:40%;height:86%;border-radius:80%; position: absolute;top:14%;left:4%;color:white;font-size:23px">Referral History</strong></nobr>
</h1>
 <div class="dropdown">
...
$sql = "SELECT * FROM funding_history WHERE email = ? AND funding_type = 'Referral commission' ORDER BY date DESC";
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<li class="transaction-item">
                ...
                <div class="transaction-details">
                    <h3>' . $funding_type . '</h3>
                    <p>₦' . number_format($amount, 2) . ' has been funded into<br> your account from admin<br> on ' . date("F j, Y", strtotime($date)) . '</p>
                </div>
