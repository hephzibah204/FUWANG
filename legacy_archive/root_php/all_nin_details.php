...
if (isset($_GET['idNumber'])) {
    $idNumber = htmlspecialchars($_GET['idNumber']); // Sanitize URL parameter
} else {
    $idNumber = null;
}
?>
...
width:100%;
  ...
  <div class="menu-container">
    ...
    <div class="dropdown-content">
          <a href="history_available_slip.php?idNumber=<?php echo $idNumber ?>"class="fa fa-download"> Download PDF</a>
          
           <a href="history_available_slip.php?idNumber=<?php echo $idNumber ?>"class="fa fa-print"> Print Slip</a>
             <a href="nin_verification_history.php"class="fa fa-dashboard"> Go To Verify History </a>
...
if (isset($_GET['idNumber'])) {
    $idNumber = $_GET['idNumber']; // Get the value from the URL parameter
    ...
    if ($result->num_rows > 0) {
        ...
        <head>
            ...
            <title>NIN Search Results</title>
    ...
    transform-origin: 0 0; /* Ensures scaling starts from the top-left corner */
        ...
        foreach ($response_data as $key => $value) {
            ...
            if ($key === 'image' || $key === 'signature') {
                ...
                $value = trim($value); // Remove any extra whitespace

                // Check for valid base64 data
                if (strpos($value, 'data:image/') === 0) {
                    // Extract MIME type from the base64 string
                    $mimeType = explode(';', explode(':', $value)[1])[0];

                    // Output image
                    echo '<tr>
                            <td>' . ucfirst($key) . '</td>
                            <td><img src="' . htmlspecialchars($value) . '" class="img-fluid" style="max-width: 200px;"/></td>
                          </tr>';
                } else {
                    echo '<tr>
                            <td>' . ucfirst($key) . '</td>
                            <td>Invalid base64 data</td>
                          </tr>';
                }
            } else {
                echo '<tr>
    ...
    echo 'Error: ID Number is not provided.';
}
