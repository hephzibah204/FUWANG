...
$sql_count = "SELECT COUNT(*) AS total_transactions FROM data_transactions_history";
$result_count = $conn->query($sql_count);
$row_count = $result_count->fetch_assoc();
$total_transactions = $row_count["total_transactions"];

// Fetch transactions with order
$sql = "SELECT * FROM data_transactions_history ORDER BY purchase_datetime DESC";
...
<body>
    <?php
include_once("data_history_bar.php");
  ...
  while($row = $result->fetch_assoc()) {
    ...
    echo '<li class="transaction-item">
      ...
      <div class="transaction-details">
        <h3>Data Purchase</h3>
        <p>'.$data_plan.' sent to '.$phone.'<br>from '.$fullname.' on '.$purchase_datetime.'</p>
      </div>
