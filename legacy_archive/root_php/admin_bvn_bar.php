...
<body>
  ...
  <div class="headers">
            ...
            <h2 style="font-size:20px;color:white; position:;margin-top:-5%;text-align:center"> <center> BVN Settings</h2>
...
$query = "SELECT nin_search_type FROM api_settings LIMIT 1";
$result = mysqli_query($conn, $query);

// Check if the query was successful and fetch the data
if ($result) {
    $row = mysqli_fetch_assoc($result);
    $nin_search_type = $row['nin_search_type'];
} else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $new_nin_search_type = $_POST['nin_search_type'];
    
    // Update the data_api_type in the database
    $update_query = "UPDATE api_settings SET
    nin_search_type = '$new_nin_search_type' WHERE id = 1"; // Assuming there's only one row to update
    if (mysqli_query($conn, $update_query)) {
        $successs[] = "NIN API Type updated successfully!";
        
        $nin_search_type = $new_nin_search_type;
    } else {
...
<body>
    <for method="post" id="airtimeApiForm"style="display:none">
        <select name="nin_search_type" onchange="autoSubmit()">
            <option selected><?php echo htmlspecialchars($nin_search_type); ?></option>
            <option value="switch off">Switch Off</option>
            <option value="search2">search2</option>
            <option value="search3">search3</option>
            <option value="search4">search4</option>
        </select>
