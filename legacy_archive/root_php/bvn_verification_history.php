...
<div style="border:1px solid #f1f1f1;">
 <a href="users_logout.php"class="fa fa-sign-out"> Logout</a>
       ...
       <center>
<div class="header">
            <h2 style="font-size:20px">Verify history</h2>
</div>
</div>
<br><br><br><br>
<?php
...
if (isset($_SESSION["email"])) {
    ...
    // SQL query to fetch all response_data from the table where email matches
    $sql = "SELECT response_data FROM id_history_bvn WHERE email = ?";
    ...
    if ($stmt = $conn->prepare($sql)) {
        ...
        if ($result->num_rows > 0) {
            // Loop through each row and extract the base64Image data and BVN
            while ($row = $result->fetch_assoc()) {
                $response_data = json_decode($row['response_data'], true);

                // Extract the BVN and base64Image data if they exist
                if (isset($response_data['bvn'])) {
                    $bvn = $response_data['bvn'];
                } else {
                    $bvn = 'BVN not found';
                }

                if (isset($response_data['base64Image'])) {
                    $base64Image = $response_data['base64Image'];
                    // Display the BVN and image
                   
                   
                     echo "<div class='content'>";
                 echo '<img src="data:image/jpeg;base64,' . htmlspecialchars($base64Image) . '" alt="BVN Image" style="max-width: 200px; max-height: 200px;border-radius:10px">';
                echo '<p>BVN: ' . htmlspecialchars($bvn) . '</p>';
                echo "
                <form action='bvn_available_slip.php' method='GET'>";
                echo "<input type='hidden' name='bvn' value='" . $bvn . "' />";
                echo "<button type='submit'>Reprint Slip</button>";
                echo "</form></div>";
            } else {
                echo "Photo: Not available<br>";
            }
            echo "<br><div style='border:3px solid #f1f1f1;'>";
        }
    } else {
        echo "0 results";
    }
