...
$sql = "SELECT fullname, number, created_time, email FROM customers1 WHERE email NOT IN (SELECT email FROM rewarded_users) ORDER BY created_time DESC";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE rewards SET rewards_amount = ? LIMIT 1";
...
$sql = "SELECT rewards_amount FROM rewards LIMIT 1";
...
<h1>
    <nobr>
<strong style="width:40%;height:86%;border-radius:80%; position: absolute;top:17%;left:4%;color:white;font-size:23px">New Users Rewards</strong></nobr>
</h1>
 <div class="dropdown">
...
<div class="modal-content">
                ...
                <center><strong>Rewards Amount</strong></center>
                
                <form  action="#" method="post">
    <center>
        <input type="tel"id="notificationInput" name="rewards_amount" style="padding:7%; background-color:#f1f1f1" placeholder="Edit ✍️ Rewards amount..."><br>
          <button style ="width:60%"type="submit"value="update">update</button>
          <hr>
        <br
        
        <small>Current Rewards Amount</small>
...
<br><br><br><br><br>
<p style="margin:2%;font-size:10px">Every New user is displaying here including with his full details, and there's have rewards button ✅  once you click on this button it automatic rewards user with the above rewards amount you've set</p>
    <div class="container">
    ...
    <input type="hidden" name="number" value="<?php echo htmlspecialchars($user['number']); ?>">
    <input type="hidden" name="created_time" value="<?php echo htmlspecialchars($user['created_time']); ?>">
    ...
    <p class="card-text">Number: <?php echo htmlspecialchars($user['number']); ?></p>
    <p class="card-text">Created Time: <?php echo htmlspecialchars($user['created_time']); ?></p>
