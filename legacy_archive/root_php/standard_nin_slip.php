...
if (isset($_GET['nin'])) {
    $nin = $_GET['nin'];
    $apiQuery = "SELECT dataverify_api_key, dataverify_endpoint_standard_slip FROM api_center LIMIT 1";
    $apiResult = $conn->query($apiQuery);

    if ($apiResult->num_rows > 0) {
        $apiData = $apiResult->fetch_assoc();
        $apiKey = $apiData['dataverify_api_key'];
        $url = $apiData['dataverify_endpoint_standard_slip'];
    } else {
    ...
    if (isset($_SESSION["email"])) {
        ...
        $stmt = $conn->prepare("SELECT nin_standard_slip_price FROM id_verification_price LIMIT 1");
        if ($stmt->execute()) {
            $stmt->bind_result($nin_standard_slip_price);
            $stmt->fetch();
            $stmt->close();
        } else {
        ...
        if ($user_balance >= $nin_standard_slip_price) {
            // Subtract the price from user balance
            $new_balance = $user_balance - $nin_standard_slip_price;
