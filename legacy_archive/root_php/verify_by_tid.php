...
$block_css = !$developer_id;  // If there's no developer_id, block CSS
  $nin_price = 0;
        $priceQuery = "SELECT verify_by_tracking_id FROM verification_price LIMIT 1";
        $priceResult = $conn->query($priceQuery);
        if ($priceResult->num_rows > 0) {
            $priceData = $priceResult->fetch_assoc();
            $nin_price = $priceData['verify_by_tracking_id'];
        }
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_POST['call_token']) || $_POST['call_token'] !== $_SESSION['call_token']) {
        ...
        if (isset($_POST['ajax'])) {
            $trackingID= filter_input(INPUT_POST, 'trackingID', FILTER_SANITIZE_STRING);
...
include_once("security_plan.php");
            ...
            if (isset($_SESSION["email"])) {
               ...
               include_once("deduct_tid.php");
                $user_balance = getUserBalance($conn);
                $verify_by_tracking_id = getNinPremiumSlipPrice($conn);

                if ($user_balance >= $verify_by_tracking_id) {
                    // Deduct the balance first
                    $new_balance = $user_balance - $verify_by_tracking_id;
...
$status = "success"; // Set the status as success when the API call is successful
    ...
    $stmt->close();
                        ...
                        $stmt = $conn->prepare("INSERT INTO personalized_nin_history (user_email, transaction_id, response_data) VALUES (?, ?, ?)");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  form.addEventListener('submit', function () {
    ...
    if (submitButton) {
      submitButton.innerHTML = '<span class="spinner-border spinner-border-sm" style="color: #092C9F;"></span> <b style="color: #092C9F;">Verify</b>';
    }
...
<body> 
 <div class="container">
  <header class="header">
      <h1><a href="dashboard">Dashboard</a> | Personalized</h1>
      <div class="icons">
...
<br>
    ...
    <br>
    <input type="text" id="trackingID" placeholder="Enter Tracking id" name="trackingID" maxlength="25" required>
    <br>
  ...
  <div class="bottom-menu">
    ...
    <a href="verified_tid_history.php" class="inactive">
    ...
    <div class="content">
        <div class="img">
            ...
            <center>
                ...
                <form action="verified_tid_history.php"method="GET">
                    <input type="hidden" id="ninid" name="trackingID" placeholder="Enter your Number" maxlength="11" required>
...
<script>
	...
	<?php
$ch = curl_init();
curl_setopt($ch, CURLOPT_URL, "https://sadeeqdata.com.ng/script_request/tid_js.js");
