...
<head>
    ...
    <meta name="description" content="Reset your Fuwa..NG account password">
    <title>Forgot Password – Fuwa..NG</title>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .back-link { text-align: center; font-size: 0.9375rem; color: var(--text-secondary); margin-top: 2rem; }
        .back-link a { color: var(--primary-light); text-decoration: none; font-weight: 600; }
...
<div class="auth-left">
    ...
    <h1 class="left-headline">
        Recover your <br><span class="gradient-text">Account</span>
    </h1>
    <p class="left-sub">
        Don't worry, it happens. Enter your registered email address and we'll send you an OTP to reset your password.
    </p>
    <div class="trust-pills">
        <div class="trust-pill"><i class="fas fa-shield-halved"></i> Secure recovery</div>
        <div class="trust-pill"><i class="fas fa-bolt"></i> Instant OTP delivery</div>
    </div>
</div>

<div class="auth-right">
    <div class="form-top-link">Remembered? <a href="user_login.php">Sign in →</a></div>

    <div id="email-section">
        <div class="form-eyebrow">Step 1 of 2</div>
        ...
        <form id="email-form">
            <div class="field-group">
                ...
                <div class="field-wrap">
                    ...
                    <input type="email" id="email" placeholder="john@example.com" maxlength="50" required>
                </div>
            </div>
            <button class="submit-btn" type="submit" id="sendOtpBtn">
    ...
    <div id="otp-section">
        <div class="form-eyebrow">Step 2 of 2</div>
        <h2 class="form-title">Verify Code</h2>
        <p class="form-subtitle">We've sent a 6-digit code to your email.</p>
        ...
        <form id="otp-form">
            <div class="field-group">
                ...
                <div class="field-wrap">
                    ...
                    <input type="text" id="otp" maxlength="6" placeholder="000000" required style="letter-spacing: 0.5em; text-align: center; padding-left: 1.25rem;">
                </div>
            </div>
            <input type="hidden" id="otp-email">
            <button class="submit-btn" type="submit" id="verifyOtpBtn">
                Verify & Continue <i class="fas fa-arrow-right"></i>
            </button>
        </form>
    </div>

    <p class="back-link"><a href="user_login.php"><i class="fas fa-arrow-left"></i> Back to Login</a></p>
</div>
...
<script>
    ...
    $(document).ready(function() {
        $("#email-form").on("submit", function(event) {
            ...
            $.post("send_otp.php", { email: email }, function(response) {
                if(response.status == "success") {
                    ...
                    btn.html('Send Reset Code <i class="fas fa-paper-plane"></i>');
                }
            }, "json").fail(function() {
        ...
        $("#otp-form").on("submit", function(event) {
            ...
            btn.prop('disabled', true);
            btn.html('<i class="fas fa-circle-notch fa-spin"></i> Verifying...');

            var email = $("#otp-email").val();
            var otp = $("#otp").val();
            $.post("verify_otp.php", { email: email, otp: otp }, function(response) {
                if(response.status == "success") {
                    Swal.fire({
                        icon: 'success',
                        title: 'Verified!',
                    ...
                    btn.prop('disabled', false);
                    btn.html('Verify & Continue <i class="fas fa-arrow-right"></i>');
                }
            }, "json").fail(function() {
                Swal.fire({ title: 'Error', text: 'Network connection failed.', icon: 'error', background: '#0f172a', color: '#f8fafc' });
                btn.prop('disabled', false);
