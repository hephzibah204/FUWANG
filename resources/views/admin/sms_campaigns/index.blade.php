...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-white">SMS Campaigns</h1>
            <p class="text-muted mb-0">Bulk SMS with provider routing and tracking.</p>
        </div>
        <a href="{{ route('admin.sms_campaigns.create') }}" class="btn btn-primary">
            <i class="fa-solid fa-comment-sms mr-2"></i> New Campaign
        </a>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-hover text-white mb-0">
                    ...
                    <tbody>
                        @forelse($campaigns as $c)
                            <tr>
                                <td class="border-0 p-3 align-middle">
                                    ...
                                    <div class="small text-white-50">{{ \Illuminate\Support\Str::limit($c->message, 70) }}</div>
                                </td>
                                <td class="border-0 p-3 align-middle">
                                ...
                                <td class="border-0 p-3 align-middle text-right">
                                    <a href="{{ route('admin.sms_campaigns.recipients', $c) }}" class="btn btn-sm btn-outline-info">
                                        <i class="fa-solid fa-users"></i>
                                    </a>
                                    @if($c->failed_count > 0)
                                        <form action="{{ route('admin.sms_campaigns.retry_failed', $c) }}" method="POST" class="d-inline" onsubmit="return confirm('Retry failed recipients for this campaign?');">
                                            @csrf
                                    ...
                                    @if(in_array($c->status, ['draft','failed']))
                                        <form action="{{ route('admin.sms_campaigns.send', $c) }}" method="POST" class="d-inline" onsubmit="return confirm('Send this SMS campaign now?');">
                                            @csrf
                        ...
                        @empty
                            <tr><td colspan="5" class="text-center p-5 text-muted">No SMS campaigns yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
