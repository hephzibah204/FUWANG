...
@section('title', 'SMS Campaign Recipients')
...
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            ...
            <p class="text-muted mb-0">{{ $smsCampaign->name }}</p>
        </div>
        <div class="d-flex" style="gap: 10px;">
            <a class="btn btn-outline-light" href="{{ route('admin.sms_campaigns.index') }}">
                <i class="fa-solid fa-arrow-left mr-2"></i> Back
            </a>
            <a class="btn btn-outline-info" href="{{ route('admin.sms_campaigns.recipients.export', $smsCampaign) }}?status={{ urlencode($status) }}&q={{ urlencode($q) }}">
                <i class="fa-solid fa-file-csv mr-2"></i> Export CSV
            </a>
            @if(($counts['failed'] ?? 0) > 0)
                <form method="POST" action="{{ route('admin.sms_campaigns.retry_failed', $smsCampaign) }}" class="d-inline" onsubmit="return confirm('Retry all failed recipients for this campaign?');">
                    @csrf
    ...
    <div class="card border-0 shadow-sm mb-3" style="background:#1e293b;">
        <div class="card-body">
            <form method="GET" action="{{ route('admin.sms_campaigns.recipients', $smsCampaign) }}">
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <label class="text-white-50 small mb-2">Search Phone</label>
                        <input type="text" name="q" value="{{ $q }}" class="form-control" placeholder="08012345678">
                    </div>
                    <div class="col-md-3 mb-3 d-flex align-items-end" style="gap: 10px;">
                        ...
                        <a class="btn btn-outline-light" href="{{ route('admin.sms_campaigns.recipients', $smsCampaign) }}"><i class="fa-solid fa-rotate-left mr-2"></i>Reset</a>
                    </div>
                </div>
            </form>
        </div>
                ...
                <table class="table table-hover text-white mb-0">
                    ...
                    <tbody>
                        @forelse($recipients as $r)
                            <tr>
                                <td class="border-0 p-3">{{ $r->phone }}</td>
