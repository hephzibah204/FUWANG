...
@section('title', 'New Admin - Fuwa.NG Control')
...
<div class="row mb-4 animate__animated animate__fadeInDown">
    <div class="col-12 d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between">
        <div>
            <h3 class="text-white mb-1 font-weight-bold"><i class="fa fa-user-plus text-primary mr-2"></i> New Admin</h3>
            <p class="text-white-50 mb-0">Create an administrator account and assign roles.</p>
        </div>
    ...
    <div class="col-lg-8 mb-4">
        <form action="{{ route('admin.admins.store') }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group mb-4">
                            ...
                            <input type="text" name="username" class="form-control bg-transparent text-white @error('username') is-invalid @enderror" value="{{ old('username') }}" required autocomplete="username">
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-4">
                            ...
                            <input type="email" name="email" class="form-control bg-transparent text-white @error('email') is-invalid @enderror" value="{{ old('email') }}" required autocomplete="email">
                            @error('email')<div class="invalid-feedback">{{ $message }}</div>@enderror
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="form-group mb-4">
                            <label class="text-white-50 small font-weight-bold text-uppercase tracking-wider">Password</label>
                            <input type="password" name="password" class="form-control bg-transparent text-white @error('password') is-invalid @enderror" required autocomplete="new-password" placeholder="Min. 8 characters">
                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-4">
                            ...
                            <input type="password" name="password_confirmation" class="form-control bg-transparent text-white" required autocomplete="new-password">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card glass-card border-0 rounded-lg p-4 mb-4" style="background: rgba(255,255,255,0.02); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.05) !important;">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="custom-control custom-switch">
                        <input type="checkbox" class="custom-control-input" name="is_super_admin" id="superAdminSwitch" value="1" {{ old('is_super_admin') ? 'checked' : '' }}>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="custom-control custom-checkbox px-0 d-flex align-items-center mb-2" style="gap: 10px;">
                                <input type="checkbox" name="roles[]" value="{{ $role->name }}" id="role_{{ $role->id }}" {{ in_array($role->name, $oldRoles, true) ? 'checked' : '' }} class="mr-2" style="width: 18px; height: 18px;">
                                <label class="text-white mb-0 text-capitalize" for="role_{{ $role->id }}">{{ str_replace('-', ' ', $role->name) }}</label>
                            </div>
                        </div>
                        @empty
                        <div class="col-12">
                            <p class="text-warning small mb-0">No Spatie roles are defined yet. Run the roles seeder or create roles first. You can still create a super admin using the switch above.</p>
                        </div>
                        @endforelse
                    </div>
                </div>
            ...
            <button type="submit" class="btn btn-primary btn-block py-3 font-weight-bold rounded-lg shadow-sm">
                <i class="fa fa-save mr-2"></i> Create Admin Account
            </button>
        </form>
    </div>
    ...
    <div class="col-lg-4">
        <div class="card glass-card border-0 rounded-lg p-4" style="background: rgba(59, 130, 246, 0.05); border: 1px solid rgba(59, 130, 246, 0.1) !important;">
            ...
            <ul class="list-unstyled text-white-50 small mb-0">
                <li class="mb-3">
                    <strong class="text-white d-block mb-1">Super Admin:</strong>
                    Full access to every module, including other admins.
                </li>
                <li class="mb-3">
                    <strong class="text-white d-block mb-1">Sub-admin:</strong>
                    Turn off Super Admin and tick the roles that match what this person should manage.
                </li>
                <li>
                    <strong class="text-white d-block mb-1">Security:</strong>
                    Share the password through a secure channel; they can change it after first login from their profile if allowed.
                </li>
            </ul>
        </div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleRoles() {
            if (!superAdminSwitch || !rolesContainer) return;
