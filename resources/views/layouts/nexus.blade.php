    ...
    <aside class="sidebar" id="sidebar" aria-label="Main Navigation" role="navigation">
        <div class="sidebar-header">
            <a href="{{ url('/') }}" class="sidebar-logo text-decoration-none">
                @if($siteLogo)
                    <img src="{{ $siteLogo }}" alt="{{ $siteName }}" loading="lazy" decoding="async" style="max-height: 32px; margin-right: 8px;">
                @else
        ...
        <nav class="sidebar-nav">
            ...
            @elseif($logisticsStaffUser && request()->routeIs('logistics.ops.*'))
            <div class="nav-section">Logistics Ops</div>
            <div class="nav-item {{ Request::routeIs('logistics.ops.dashboard') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.dashboard') }}"><i class="fa-solid fa-gauge-high"></i> <span class="nav-text">Dashboard</span></a>
            </div>
            <div class="nav-item {{ Request::routeIs('logistics.ops.orders.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.orders.index') }}"><i class="fa-solid fa-boxes-stacked"></i> <span class="nav-text">Orders</span></a>
            </div>
            @if(method_exists($logisticsStaffUser, 'hasPermission') && $logisticsStaffUser->hasPermission('logistics.shipments.monitor'))
            <div class="nav-item {{ Request::routeIs('logistics.ops.shipments.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.shipments.index') }}"><i class="fa-solid fa-truck-fast"></i> <span class="nav-text">Shipments</span></a>
            </div>
            @endif
            @if(method_exists($logisticsStaffUser, 'hasPermission') && $logisticsStaffUser->hasPermission('logistics.agents.view'))
            <div class="nav-item {{ Request::routeIs('logistics.ops.agents.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.agents.index') }}"><i class="fa-solid fa-people-carry-box"></i> <span class="nav-text">Agents</span></a>
            </div>
            @endif
            @if(method_exists($logisticsStaffUser, 'hasPermission') && $logisticsStaffUser->hasPermission('logistics.centers.manage'))
            <div class="nav-item {{ Request::routeIs('logistics.ops.centers.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.centers.index') }}"><i class="fa-solid fa-warehouse"></i> <span class="nav-text">Centers</span></a>
            </div>
            @endif
            @if(method_exists($logisticsStaffUser, 'hasPermission') && $logisticsStaffUser->hasPermission('logistics.inventory.view'))
            <div class="nav-item {{ Request::routeIs('logistics.ops.inventory.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.inventory.index') }}"><i class="fa-solid fa-boxes-packing"></i> <span class="nav-text">Inventory</span></a>
            </div>
            @endif
            @if(method_exists($logisticsStaffUser, 'hasPermission') && $logisticsStaffUser->hasPermission('logistics.analytics.view'))
            <div class="nav-item {{ Request::routeIs('logistics.ops.analytics.*') ? 'active' : '' }}">
                <a href="{{ route('logistics.ops.analytics.index') }}"><i class="fa-solid fa-chart-pie"></i> <span class="nav-text">Analytics</span></a>
            </div>
            @endif
            @elseif($webUser)
        ...
        <nav class="navbar navbar-expand-lg navbar-dark fixed-top public-nav py-3" style="background: rgba(3, 7, 18, 0.95); backdrop-filter: blur(15px); border-bottom: 1px solid rgba(255,255,255,0.05); z-index: 1050;">
            <div class="container">
                <a class="navbar-brand font-weight-bold d-flex align-items-center" href="{{ url('/') }}">
                    @if($siteLogo)
                        <img src="{{ $siteLogo }}" alt="{{ $siteName }}" loading="lazy" decoding="async" style="height: 36px; width: 36px; object-fit: contain; margin-right: 10px;">
                    @else
